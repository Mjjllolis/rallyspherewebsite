  ...
  return (
    ...
    >
      <Container className="!px-0 !max-w-none">
        ...
        {isOpen && (
          <div className="md:hidden surface-navy text-white px-6 py-6 space-y-6 mt-3 rounded-3xl">
            {menuItems.map((item) => (
              <Link key={item.text} href={item.url} className="block text-white hover:text-accent-via transition-colors py-2 text-lg" onClick={() => setIsOpen(false)}>
                {item.text}
              </Link>
            ))}
            <Link
              href="#cta"
              className="block text-center text-white brand-gradient hover:opacity-90 px-6 py-3 rounded-full transition-opacity text-lg mt-4"
